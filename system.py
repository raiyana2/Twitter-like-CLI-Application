...
Key components:
1. User Authentication (signIn, register)
2. Tweet Management (composeTweet, retweet)
3. Search Functionality (searchTweets, searchUsers)
4. Feed Display (showFollowedTweets)

Database Schema:
- users: User account information
- tweets: Tweet content and metadata
- follows: User follow relationships
- hashtag_mentions: Hashtag tracking
- retweets: Retweet tracking with spam detection

Collaborators: Raiyana Rahman, Utsha Samantha, Sydney Smythe, Faiaz Bin Nesar
"""
...
def signIn():
    ...
    Validates user ID and password against database.
    """
...
def register():
    ...
    Validates email format, phone number, and generates unique user ID.
    """
    global connection, c, loggedInUser
    while(1):
...
def homeScreen():
    ...
    Displays feed and handles user actions.
    """
    global connection, c, loggedInUser
...
def showMoreTweetsInFeed():
    """
    Implements pagination for the user's feed.
    Shows next 5 tweets and handles user interaction with displayed tweets.
    """
    ...
    SELECT DISTINCT t.tid,        -- Tweet ID
           ...
           CASE 
               WHEN r.retweeter_id IN (SELECT flwee FROM follows WHERE flwer = ?) 
               THEN r.retweeter_id 
               ELSE NULL 
           END as retweeter_id,  -- Only show retweeter if you follow them
           CASE 
               WHEN r.retweeter_id IN (SELECT flwee FROM follows WHERE flwer = ?) 
               THEN u2.name 
               ELSE NULL 
           END as retweeter_name, -- Only show retweeter name if you follow them
           CASE 
               WHEN r.retweeter_id IN (SELECT flwee FROM follows WHERE flwer = ?) 
               THEN r.rdate 
               ELSE NULL 
           END as retweet_date   -- Only show retweet date if you follow retweeter
    FROM tweets t
    JOIN users u1 ON t.writer_id = u1.usr
    LEFT JOIN retweets r ON t.tid = r.tid 
    LEFT JOIN users u2 ON r.retweeter_id = u2.usr
    WHERE 
       t.writer_id IN (SELECT flwee FROM follows WHERE flwer = ?)     -- Show tweets from people you follow
       OR r.retweeter_id IN (SELECT flwee FROM follows WHERE flwer = ?) -- OR retweets by people you follow
    ORDER BY 
    ...
    while True:
        # currentResults = c.execute(query, (loggedInUser, loggedInUser, loggedInUser, loggedInUser, loggedInUser, startPosition)).fetchall()    
        currentResults = c.execute(query, (loggedInUser, loggedInUser, loggedInUser, loggedInUser, loggedInUser, startPosition)).fetchall() 
...
def showFollowedTweets(): 
    """
    Displays user's feed: tweets from followed users and their retweets.
    Shows retweet information only when the retweeter is followed.
    All data processing is done in SQL.
    """  
    global connection, c, loggedInUser
    ...
    #In the SQL query, we have five ? placeholders that need the loggedInUser value
    allResults = c.execute(query, (loggedInUser, loggedInUser, loggedInUser, loggedInUser, loggedInUser)).fetchall()
...
def searchTweets(): 
    """
    Searches tweets based on comma-separated keywords.
    
    Functionality:
    - Supports both text and hashtag searches
    - Shows 5 results per page with pagination
    - Allows returning to home screen
    
    Returns:
        None. Displays search results or appropriate messages.
    """

    global connection, c, loggedInUser
    #the while loop Keeps prompting for new searches if no matches are founds
    while True:
        print("\nEnter keywords to search (separate multiple keywords with ','):")
        ...
        keywords = [k.strip() for k in keywords_input.split(',')]  # Split by comma and strip each keyword
        if not keywords:
...
def showTweetDetails(tweet_id): 
    """
    Displays detailed information about a specific tweet.
    Args:
    ...
    """
    # Explanation:
    # - Joins tweets with users to get writer information
    # - Subquery counts replies by checking replyto_tid
    # - Subquery counts retweets from retweets table
    # - Results ordered by date and time for consistency
...
def retweet(tweet_id):  
    ...
    Includes spam detection for repeated retweets.
    """
    global connection, c, loggedInUser
    ...
    try:
        ...
        # Insert the retweet with retweeter_id (not spam since it's first time)
        current_date = time.strftime("%Y-%m-%d")  # Define current_date here
        c.execute("""
            INSERT INTO retweets (retweeter_id, tid, writer_id, rdate, spam) 
            VALUES (?, ?, ?, ?, ?)
        """, (loggedInUser, tweet_id, original_writer_id, current_date, 0)) 
        connection.commit()        
...
def searchUsers():
    ...
    Implements pagination and prioritizes exact matches.
    """
    global connection, c, loggedInUser
    print('Search Users')
...
def showUserDetails(user_id):
    ...
    Shows user stats and recent tweets with follow options.
    """
    global connection, c, loggedInUser
...
def listFollowers(user_id):
    """
    Displays paginated list of user's followers with interaction options.
    Shows 5 followers per page with detailed information.
    """
    global connection, c, loggedInUser
    print("\nList Followers")
...
def followUser(follower_id, followee_id):
    """
    Establishes a follow relationship between users.
...
def showMoreTweets(user_id):
    """
    Shows additional tweets from a user beyond the initial display.
    ...
    tweets_query = """
        ...
        OFFSET 3;  -- Skip the first 3 tweets that were already shown
...
def composeTweet(replyTo = None):
    """
    Compose and post a new tweet.
    Handles hashtag validation and prevents duplicate hashtags.
    """
    global connection, c, loggedInUser
...
def checkRepeatedHashtags(tagList):
...
def main():
    ...
    Handles database connection and user session initialization.
    """
    global connection, c
    if(len(sys.argv) != 2):
